...
public class HoustonMotionProfileExecutor {
	 ...
	 */
	public HoustonMotionProfileExecutor(TalonSRX talon, TalonSRX follower) {
		_talon = talon;
	...
	/*
	 * * alternative way to execute MP outside of robot loop start the thread in
	 * cmd's initialize(). stop it in end() && interrupted() Make sure to comment
	 * out the execute() calls if you are going to do it this way. public void
	 * startWorking(boolean movingUp) { workerThread = new Thread(() -> { while
	 * (!Thread.interrupted()) { control(movingUp); setMotionProfileMode(); }
	 * System..out.println("Exited motion profile thread"); });
	 * workerThread.start(); }
	 * 
	 * public void stopWorking() { if (workerThread != null) {
	 * workerThread.interrupt(); System..out.println("Sent interrupt signal"); }
	 * else { System..out.println("worker thread null"); } }
	 */

	/**
	 * Called every loop.
	 */
	public void control(PodAction action, ClimbLevel level) {
	...
	private void startFilling(PodAction action, ClimbLevel level) {
		if (level == ClimbLevel.LEVEL3) {
			if (action == PodAction.CLIMB)
				startFilling(GeneratedLevel3Up.Points, GeneratedLevel3Up.kNumPoints);
			else
				startFilling(GeneratedLevel3Down.Points, GeneratedLevel3Down.kNumPoints);
		} else {
			// LEVEL2 CLIMB
			if (action == PodAction.CLIMB)
				startFilling(GeneratedLevel2Up.Points, GeneratedLevel2Up.kNumPoints);
			else
				startFilling(GeneratedLevel2Down.Points, GeneratedLevel2Down.kNumPoints);
		}
	}
	...
	public void stopMotionProfile() {
		...
		System.out.println(
				"Stopped motion profile: " + _setValue + ": " + ec + ": " + _talon.getSelectedSensorPosition());
		// _setValue = SetValueMotionProfile.Disable;
		// reset();
	}
